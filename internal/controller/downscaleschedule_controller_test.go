...
var _ = Describe("DownscaleSchedule Controller", func() {
	Context("When reconciling a resource", func() {
		...
		downscaleschedule := &downscalerv1alpha1.DownscaleSchedule{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind DownscaleSchedule")
			err := k8sClient.Get(ctx, typeNamespacedName, downscaleschedule)
			if err != nil && errors.IsNotFound(err) {
				resource := &downscalerv1alpha1.DownscaleSchedule{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance DownscaleSchedule")
